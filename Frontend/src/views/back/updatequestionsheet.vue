  ...
  methods: {
    ...
    async getchooseproblem() {
      const { data: res } = await this.$ajax({
        ...
        data: {
          question_sheet_id: this.onedata.id,
        },
      }).catch((t) => {
        this.$msg({
          type: 'error',
    ...
    async getdata() {
      const { data: res } = await this.$ajax({
        method: 'post',
        url: '/QuestionSheet/backLookOneQuestionSheetData',
        portType: {
          process: '8796',
        },
        data: {
    ...
    async updateOneQuestionSheet() {
      if (this.isup) {
        this.$msg({
          type: 'error',
          message: '正在更新题单，请耐心等待！',
          duration: 1600,
          offset: 80,
        });
        return;
      ...
      const { data: res } = await this.$ajax({
        method: 'post',
        url: '/QuestionSheet/updateOneQuestionSheet',
        portType: {
          process: '8796',
        },
        data: {
          data: this.onedata,
          question_sheet_data: resproblem,
        },
      }).catch((t) => {
        this.isup = false;
        this.defaultnum = 0;
